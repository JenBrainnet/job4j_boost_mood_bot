...
public class MoodLogFakeRepository
    ...
    public List<User> findUsersWhoDidNotVoteToday(long startOfDay, long endOfDay) {
        return memory.values().stream()
                .map(MoodLog::getUser)
                .distinct()
                .filter(user -> memory.values().stream()
                        .noneMatch(moodLog -> moodLog.getUser().getId().equals(user.getId())
                                && moodLog.getCreatedAt() >= startOfDay
                                && moodLog.getCreatedAt() <= endOfDay))
                .collect(Collectors.toList());
    }
    ...
    @Override
    public List<MoodLog> findByUserIdAndCreatedAtGreaterThanEqual(Long userId, long createdAt) {
        return List.of();
    }
